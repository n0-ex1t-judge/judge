...
class Login
{
        ...
        if (!empty($_POST['user_name']) && !empty($_POST['user_password'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                if ($checklogin->num_rows == 1) {
                    ...
                    if (crypt($_POST['user_password'],"vladi") == $result_row->user_password_hash) {
                        ...
                        $this->errors[] = "Грешна парола. Моля опитайте отново.";
                    }

                } else {

                    $this->errors[] = "Въведеният потребител не съществува.";
                }

            } else {

                $this->errors[] = "Неуспешна връзка с базата данни.";
            }

        } elseif (empty($_POST['user_name'])) {

            $this->errors[] = "Полето за потребителско име е празно.";

        } elseif (empty($_POST['user_password'])) {

            $this->errors[] = "Полето за парола е празно.";
        }
    }
    ...
    public function doLogout()
    {
        ...
        $this->messages[] = "Вие успешно излязохте от профила си.";
